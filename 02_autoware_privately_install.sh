...
# Check if folder existsautoware.privately-owned-vehicles exists, if yes rename it to autoware.privately-owned-vehicles-old
...
if [ -d "$SOURCE" ]; then
    ...
    echo "❌ Error: autoware_projects folder does not exist"
    exit 1
fi

# Fix ownership and permissions if running as sudo
if [ -n "$SUDO_USER" ]; then
    chown -R $SUDO_USER:$SUDO_USER autoware_projects
    echo "✅ Ownership set to $SUDO_USER"
fi
